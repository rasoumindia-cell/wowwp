...
/** LAN / custom hosts for `npm run dev:mobile` (Next.js blocks them otherwise). */
function getAllowedDevOrigins(): string[] {
  const origins = new Set<string>();
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL;
  if (siteUrl) {
    try {
      origins.add(new URL(siteUrl).hostname);
    } catch {
      /* ignore invalid URL */
    }
  }
...
const nextConfig: NextConfig = {
  ...
  // Hide the Next.js dev route/compiler badge on dashboard (and all pages).
  devIndicators: false,
  /**
