  ...
  return (
    <section className="flex h-full flex-col rounded-xl border border-border bg-card transition-colors duration-200">
      <header className="border-b border-border px-5 py-4">
        <h2 className="text-sm font-semibold text-foreground transition-colors duration-200">Pipeline Value</h2>
        <p className="mt-0.5 text-xs text-muted-foreground transition-colors duration-200">
          Open deals by stage
        </p>
      </header>
              ...
              {data.stages.map((s) => (
                <li key={s.id} className="flex items-center gap-3 text-xs">
                  <span
                    ...
                    aria-hidden
                  />
                  <span className="flex-1 truncate text-foreground transition-colors duration-200">{s.name}</span>
                  <span className="text-muted-foreground tabular-nums transition-colors duration-200">
                    {s.dealCount} deal{s.dealCount === 1 ? '' : 's'}
                  </span>
                  <span className="w-20 text-right text-foreground tabular-nums transition-colors duration-200">
    ...
    <div className="flex items-center justify-center">
      <svg viewBox={`0 0 ${size} ${size}`} className="h-48 w-48" role="img" aria-label="Pipeline value by stage">
        ...
        <text
          ...
          y={cy + 14}
          textAnchor="middle"
          className="fill-current text-foreground text-[18px] font-semibold tabular-nums"
        >
