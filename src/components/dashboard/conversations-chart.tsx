...
export function ConversationsChart({ series, loading, range, onRangeChange }: ConversationsChartProps) {
  ...
  return (
    <section className="flex h-full flex-col rounded-xl border border-border bg-card transition-colors duration-200">
      <header className="flex items-center justify-between border-b border-border px-5 py-4">
        <div>
          <h2 className="text-sm font-semibold text-foreground transition-colors duration-200">Conversations Over Time</h2>
          <p className="mt-0.5 text-xs text-muted-foreground transition-colors duration-200">Daily message volume by direction</p>
        </div>
        <div className="flex items-center gap-1 rounded-lg bg-muted p-1 transition-colors duration-200">
          {[7, 30, 90].map((r) => (
            <button
              key={r}
              type="button"
      ...
      <footer className="flex items-center gap-4 border-t border-border px-5 py-3 text-xs text-muted-foreground transition-colors duration-200">
...
function LineSvg({
  ...
  return (
    <div ref={wrapRef} className="relative w-full">
          ...
          return (
            <g key={t}>
              ...
              <text
                ...
                className="fill-current text-muted-foreground text-[10px]"
              >
                {t}
              </text>
            </g>
      ...
      {hovered && hover !== null && (
        <div
          className="pointer-events-none absolute top-0 z-10 -translate-x-1/2 rounded-md border border-border bg-popover px-2.5 py-1.5 text-[11px] shadow-lg transition-colors duration-200"
        ...
        >
          ...
          <div className="mt-1 flex flex-col gap-0.5">
            ...
            <span className="flex items-center gap-1.5 text-violet-500 dark:text-violet-300">
