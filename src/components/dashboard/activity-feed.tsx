...
export function ActivityFeed({ items, loading }: ActivityFeedProps) {
  ...
  return (
    <section className="rounded-xl border border-border bg-card transition-colors duration-200">
      <header className="flex items-center justify-between border-b border-border px-5 py-4">
        <h2 className="text-sm font-semibold text-foreground transition-colors duration-200">Recent Activity</h2>
        <Link
          href="/inbox"
      ...
      {loading || !items ? (
        ...
        <>
            ...
            {visible.map((it, i) => {
              ...
              const row = (
                <div className="flex items-center gap-3 px-5 py-2.5">
                  ...
                  <span className="min-w-0 flex-1 truncate text-sm text-foreground transition-colors duration-200">
                    {it.text}
                  </span>
                  <span className="flex-shrink-0 text-xs text-muted-foreground tabular-nums transition-colors duration-200">
                    {relativeTime(it.at)}
                  </span>
                </div>
              )
          ...
          <footer className="flex items-center justify-between border-t border-border px-5 py-3 text-xs transition-colors duration-200">
            <span className="text-muted-foreground tabular-nums transition-colors duration-200">
              Showing {visible.length} of {totalLoaded}
              {totalLoaded === 50 ? '+' : ''}
            </span>
            <div className="flex items-center gap-1">
              <span className="mr-1 text-muted-foreground transition-colors duration-200">Show</span>
